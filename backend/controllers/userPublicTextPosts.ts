...
export const createPublicTextPosts = async (req: Request, res: Response) => {
...
export const getMyPublicTextPosts = async (req: Request, res: Response) => {
    const userId = req.user?.userId;
...
export const getAllPublicPosts = async (_req: Request, res: Response) => {
    try {
        const [posts] = await pool.query<RowDataPacket[]>(
            `SELECT 
                p.id AS postId,
