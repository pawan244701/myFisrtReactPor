...
export const userLogin = async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(
            "SELECT * FROM users WHERE username = ? OR password = ? LIMIT 1",
            [username, password] // always pass the exect same incoming values to query o/w you'll get an error: 500 server error
        );
        if (rows.length === 0) {
